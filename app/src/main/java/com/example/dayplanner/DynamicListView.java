...
public class DynamicListView<T> extends ListView {
    ...
    private long mIdToDelete = -1;

    private DbUpdater<T> dbUpdater;

    private ImageView sideBars;
    ...
    private AdapterView.OnItemLongClickListener mOnItemLongClickListener =
            new AdapterView.OnItemLongClickListener() {
                public boolean onItemLongClick(AdapterView<?> arg0, View arg1, int pos, long id) {
                    ...
                    //sideBars.setVisibility(VISIBLE);
                    sideBars.setScaleX(0.8f);
                    sideBars.setScaleY(0.8f);
                    sideBars.animate().alpha(1.f).setDuration(300).scaleX(1.f).scaleY(1.f).setInterpolator(new OvershootInterpolator()).start();
    ...
    private void handleCellSwitch() {
        ...
        if (isBelow || isAbove) {
            ...
            swapElements(namesList, originalItem, getPositionForView(switchView));
        ...
        int width = mHoverCellOriginalBounds.width();

        if (mLastEventX<=width/10){
            Context c = getContext();
            mIdToDelete = mMobileItemId;
            AlertDialog dialog = new AlertDialog.Builder(c).setTitle("Delete").setMessage("Are you sure you want to delete this item?").setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {
                    dbUpdater.delete(namesList,mIdToDelete);
                    dialog.dismiss();
                }
        ...
        if (mLastEventX>=width-width/10){
            dbUpdater.edit(getPositionForID(mMobileItemId));
            touchEventsCancelled();
        }
    }
    ...
    private void touchEventsEnded () {
        ...
        //sideBars.setVisibility(INVISIBLE);
        sideBars.animate().alpha(0.f).setDuration(200);
    }
    ...
    public void setDbUpdater(DbUpdater<T> updater,ArrayList<T> list,ImageView sideBArs){
        dbUpdater = updater;
        namesList=list;
        sideBars = sideBArs;
    ...
    public DbUpdater<T> getDbUpdater(){
        return dbUpdater;
    }
